...
B. EXERCISE LOOP (per exercise)
   ...
   - Call calibration_check(concept, predicted_mastery) only for session-opening calibration, mastery challenges, transfer/feynman probes, or every few exercises when calibration is stale. Do not block every routine exercise on a self-rating.
	...
	- For a decision-bound attempt, rubric_score_json must use criteria_scores with each exact frozen criterion id, a finite numeric score and non-empty evidence exactly once. Use canonical fields, not aliases or numeric strings. Duplicate JSON keys, unsupported rules and contradictory total/max_total/max_score fields are rejected. The runtime derives pass/fail without an approximate passing tolerance; optional confidence is not evaluator trust.
   After:
