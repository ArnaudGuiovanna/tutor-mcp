...
func TestIntegrationSecretEncryptedAtRestAndDecryptedAtUse(t *testing.T) {
	...
	got, err := s.GetLearnerByID(context.Background(), learner.ID)
	if err != nil || got.WebhookURL != secret {
		t.Fatalf("decrypted learner=%+v err=%v", got, err)
	}
}
...
func TestIntegrationSecretRotationSupportsOldKeyWithoutDowntime(t *testing.T) {
	...
	if got, err := s.GetLearnerByID(context.Background(), learner.ID); err != nil || got.WebhookURL != secret {
		t.Fatalf("read with retained old key: learner=%+v err=%v", got, err)
	}
	...
	if got, err := s.GetLearnerByID(context.Background(), learner.ID); err != nil || got.WebhookURL != secret {
		t.Fatalf("read after rotation: learner=%+v err=%v", got, err)
	}
}
...
func TestIntegrationSecretRotationEncryptsValidatedLegacyRows(t *testing.T) {
	...
	if got, err := s.GetLearnerByID(context.Background(), "legacy-secret"); err != nil || got.WebhookURL != secret {
		t.Fatalf("legacy read learner=%+v err=%v", got, err)
	}
}
...
func TestIntegrationSecretWrongKeyFailsWithoutDisclosingSecret(t *testing.T) {
	...
	_, err = s.GetLearnerByID(context.Background(), learner.ID)
